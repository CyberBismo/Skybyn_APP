...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        try {
          _posts = await PostService().fetchPostsForUser(userId: userId)
  ...
  Future<void> _performCommentDeletion(String commentId) async {
    try {
      final userId = await _authService.getStoredUserId();
      if (userId == null) {
      ...
      final postWithComment = _posts.firstWhere(
        (post) => post.commentsList.any((comment) => comment.id == commentId),
        orElse: () => throw Exception('Comment not found in any post'),
      );
      ...
      // Update the post to reflect the deletion
      await _updatePost(postWithComment.id);
  ...
  void _scrollToFocusedPost() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final finalPosition = (targetPosition + additionalPadding).clamp(
        0.0, 
